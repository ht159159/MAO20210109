<template>
  <div class="promotion">
    <div class="promo" v-show="!show">
      <header-cmp :title="$t('Promotion_Activity')" :isBack="false"></header-cmp>
      <div class="nav">
        <ul>
          <li v-for="(p, i) in promotion" :key="p" :class="{ active: i === selected }">
      ...
      <div class="main" v-if="activitys.length">
        <div class="item-group" v-for="act in activitys" :key="act.id">
          <div class="pic" @click.prevent="showDetails(act.activityName,act.mImgUrl)">
            <img :src="act.imgUrl" alt />
          </div>
          <div class="title">
            <h1>{{ act.activityName }}</h1>
            <p>{{ act.beginTime }}{{ $t("to") }}{{ act.endTime }}</p>
          </div>
        </div>
      </div>
